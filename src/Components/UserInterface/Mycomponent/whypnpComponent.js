...
export default function WhypnpComponent(props){
   ...
   var myslider=createRef()
    ...
    const playSlide = () => {
        return whypnp.map((item) => {
            return (
                <div key={item.offerid}>
                    <div style={{justifyItems:'center',position:'relative',display:'flex',width:285,height:108,borderRadius:20,background:'#fff', paddingLeft: 10, paddingRight: 10 }}>
                    <span style={{position:'absolute',height:"60px",width:"60px",left:"20px",top:'30%',display:'inline-block',}}>
                      <img  src={`${ServerURL}/images/${item.image}`} style={{ height:"100%",width:"100%",objectFit:'contain'}} />
                    </span>
                      <h style={{fontSize:20,fontWeight:700,color:'#2d3436',left:95,top:12,position:"absolute"}}>
                          {item.title}
                      </h>
      ...
      return(
         ...
         <div style={{display:'flex',justifyContent:'space-between',paddingBottom:10,paddingTop:10,width:'96%'}}>
          <span style={{fontWeight:"bolder",fontSize:28,color:'#fff'}}>Whypnp</span>
