...
export default function DisplayAllVehicle(props){
  ...
  const fetchAllCompanyBySubCategory=async(subcategory_id)=>{
   ...
   const fetchAllModelByCompany=async(company_id)=>{
     ...
     var result=await postData('model/fetch_all_model_by_company',body)
     setModelList(result.data)
   }
 ...
 const handleEditData=async()=>{
  var body={vehicleid:vehicleId,categoryid:categoryId,subcategoryid:subcategoryId,companyid:companyId,modelid:modelId,vendorid:vendorId,registrationno:registrationNo,color:color,fueltype:fueltype,ratings:ratings,average:average,remark:remarks,capacity:capacity,status:status,feature:feature}
